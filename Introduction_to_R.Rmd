...
*Written by Keaka Farleigh on August 5th, 2025: edited from materials originally developed by Dr. Tereza Jezkova and Dr. Alfredo Ascanio*

*Last updated by Keaka Farleigh on July 2nd, 2026* 
...
- Learn what R is.
- Learn the advantages and disadvantages of R.
- Learn what Rstudio is and how to navigate it.
- Learn how to load a software package and get help.
- Learn the data types and structures used in R.
- Learn the elements of a function.
...
Variables (also called objects) are assigned with the `<-` or `=`. Either work, but I prefer `<-` for stylistic reasons. These variables are then stored in the global environment after you create them. 

```{r assign variables, echo = TRUE, eval = FALSE}
...
Data in R can take a few forms and be store in a few different structures. We list them below and provide examples of each one. 

### Data types

- Numeric: numbers with decimals
- Integers: whole numbers
- Characters: strings, like "abc"
- Logical: TRUE or FALSE
- Complex: Formulas, including imaginary numbers (1 + 1i)
- Factors: A categorical classification of data

### Data structures

- Vector: 1 dimensional object containing data of the same type (e.g., all numbers)
- Matrix: 2 dimensional (rows and columns) object containing data of same type (e.g., all numbers)
- Dataframe: 2 dimensional (rows and columns) object containing data of same or different types (e.g., one column of characters and one of numbers)
- List: 1 dimensional object that can contain different data structures. Each object in a list is called an element. A list, for example could contain vector, matrix, and data frame elements. 

## Function elements

A function is a bundle of code that you or someone else has written to make an analysis easier. The function will consist of the name of the function and arguments. Arguments tell the function which data to use and how to use it. The `mean` function below, for example, uses the `x` argument to determine which data to take the mean of. 

```{r function elements, echo = TRUE, eval = FALSE}

mean(x = c(10,20,30,40))

```

Thanks for reading this chapter, please reach out if you have any questions!
